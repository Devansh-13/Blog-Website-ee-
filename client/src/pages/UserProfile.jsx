...
const UserProfile = () => {
  ...
  //redirect to login page for any user who is'nt logeg in 
  useEffect(() =>{
    if(!token){
      navigate('/login')
    }
  ...
  const updateUserDetails = async (e) => {
    ...
    const response = await axios.patch(`${process.env.REACT_APP_BASE_URL}/users/edit-users`
    , userData, {withCredentials: true, headers:{Authorization: `Bearer ${token}`}});
    if(response.status == 200){
      //log user out and login with new detaials
      navigate('/logout')
  }
    } 
    catch (error) {
      ...
      <div className='container profile__container'>
        ...
        <div className="profile__details">
          ...
          <form className="form profile__form" onSubmit={updateUserDetails}>
            ...
            <input type="password" placeholder='Current Passward' value={currentPassword} 
            onChange={e=> setCurrentPassword(e.target.value)} />
            <input type="password" placeholder='New Passward' value={newPassword} 
            onChange={e=> setNewPassword(e.target.value)} />
            <input type="password" placeholder='Confirm Passward' value={confirmNewPassword} 
